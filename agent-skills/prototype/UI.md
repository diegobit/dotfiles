...
### 6. Report the answer and cleanup path

Once a variant has won, report which variant won, why, and the exact cleanup/promotion path:

- **Sub-shape A**: the proposed follow-up is to fold the winner into the existing page and drop the losing variants and switcher.
- **Sub-shape B**: the proposed follow-up is to promote the winner to a real route and drop the throwaway route and switcher.

Perform that production integration, cleanup, or branch preservation only when the user separately asks. Until then, leave the prototype clearly marked and report its files.
