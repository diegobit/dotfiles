...
Once the prototype has answered its question, report the verdict, the artifact path, and which reducer, machine, or function shape should carry into production. Preserve it in a branch, plan, or production module only when the user separately asks; follow the mutation boundary in the [SKILL](SKILL.md).
